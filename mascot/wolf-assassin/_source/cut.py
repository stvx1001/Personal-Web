# Usage (run in this folder): python3 upscale.py sheet.webp sheet4x.png realesrgan-x4plus-anime
#   then  HOLES=<mode> STRINGS=<0|1> python3 cut.py sheet4x.png 4 <outdir>
# HOLES=yellow: an enclosed hole is background only if it is cream-yellow (keeps the cat's white T-shirt solid)
# HOLES=all:    every enclosed hole is background (bird: gaps between bow, cloak and arrows)
# STRINGS=1:    keep thin lines the colour of a cast shadow (bow strings) via a black top-hat
# CRYSTALS=1:   the Fox Mage staff-crystal fill (off unless set)
import os
...
# STRINGS=1: thin dark lines the colour of a cast shadow (bow strings) -- a black top-hat keeps
# anything narrower than ~4px (1x) that is darker than its surroundings; broad soft shadows score ~0
if os.environ.get('STRINGS') == '1':
    th = nd.grey_closing(k, size=(4 * S + 1, 4 * S + 1)) - k
    alpha = np.maximum(alpha, np.clip((th - 0.05) / 0.06, 0, 1))
solid = alpha > 0.5
...
if nh:
    ...
    yellow = nd.mean(im[..., 2] - im[..., 0], holes, range(1, nh + 1))  # cream gap ~-15, white shirt ~-5
    greeny = nd.mean(im[..., 1] - im[..., 2], holes, range(1, nh + 1))  # cream gap ~+11, shaded pink shirt ~+2
    keep_open = [i + 1 for i in range(nh) if sizes[i] > 25 * S * S and (True if os.environ.get('HOLES') == 'all' else near_bg[i] > 0.6 if os.environ.get('HOLES') != 'yellow' else (yellow[i] < -9 and greeny[i] > 4))]
...
import os
for i, sl in (enumerate(nd.find_objects(cand), 1) if os.environ.get('CRYSTALS') == '1' else []):
