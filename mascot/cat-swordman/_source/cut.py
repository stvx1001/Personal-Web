# Usage: HOLES=yellow python3 cut.py sheet4x.png 4 <outdir>   (run in this folder)
# HOLES=yellow: an enclosed hole is background only if it is cream-yellow, so the white
# T-shirt stays solid. CRYSTALS=1 re-enables the Fox Mage staff-crystal fill (off here).
import os
...
if nh:
    ...
    keep_open = [i + 1 for i in range(nh) if sizes[i] > 25 * S * S and (near_bg[i] > 0.6 if os.environ.get('HOLES') != 'yellow' else (yellow[i] < -9 and greeny[i] > 4))]
